...
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='每日签到表';

-- 正在导出表  paper-cutting.daily_sign 的数据：~2 rows (大约)
...
-- 正在导出表  paper-cutting.order_info 的数据：~1 rows (大约)
INSERT INTO `order_info` (`order_id`, `goods_id`, `buyer_id`, `receiving_address_id`, `order_status`, `create_time`, `update_time`, `del_flag`) VALUES
	(1, 1, 1782381108064657409, 1, 3, '2024-04-24 11:47:50', '2024-04-24 11:47:51', b'0');
...
-- 正在导出表  paper-cutting.receiving_address 的数据：~1 rows (大约)
INSERT INTO `receiving_address` (`receiving_address_id`, `buyer_id`, `recipient_name`, `recipient_phone`, `recipient_region`, `recipient_address`) VALUES
	(1, 1782381108064657409, '收件人姓名1', '收件人电话', '收件人所在地区', '收件人详细地址');
...
) ENGINE=InnoDB AUTO_INCREMENT=722354188 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='店铺信息表';

-- 正在导出表  paper-cutting.shop_info 的数据：~7 rows (大约)
...
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8mb3 ROW_FORMAT=DYNAMIC COMMENT='后台用户表';

-- 正在导出表  paper-cutting.ums_admin 的数据：~8 rows (大约)
...
CREATE TABLE IF NOT EXISTS `ums_admin_login_log` (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=423 DEFAULT CHARSET=utf8mb3 ROW_FORMAT=DYNAMIC COMMENT='后台用户登录日志表';
...
-- 正在导出表  paper-cutting.ums_admin_role_relation 的数据：~6 rows (大约)
...
-- 正在导出表  paper-cutting.ums_admin_shop_relation 的数据：~0 rows (大约)
